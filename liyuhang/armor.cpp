...
bool ArmorDetector::DetectArmor(Mat &img,Rect roi_rect)
{
    ...
    Mat binary_brightness_img, binary_color_img, gray,hsv_img,dst_image;
    cvtColor(img, hsv_img, COLOR_BGR2HSV);          //色彩空间转换成HSV通道
    ...
    cvtColor(dst_image,gray,COLOR_BGR2GRAY);
    vector<cv::Mat> bgr;
    split(img, bgr);
    Mat result_img;
    if(color_ == 0)
    ...
    blur(result_img, result_img, Size(1,3)); 
...
//#endif
    ...
    if(found_flag)
    {
        ...
        if(cap_mode_ == 0)
        {
            offset_point = Point2f(90, 90) - Point2f(short_offset_x_,short_offset_y_);
        }
        else
        {
            offset_point = Point2f(90, 90) - Point2f(long_offset_x_,long_offset_y_);
        }
