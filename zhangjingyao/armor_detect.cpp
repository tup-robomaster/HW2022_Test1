...
{
    rectangle(img, rect + Point_<int>(roi_offset_point), Scalar(maxx,maxx,maxx), 1);
}

void armor::draw_spot(Mat &img, Point2f roi_offset_point) const
{
    circle(img, center + Point_<int>(roi_offset_point), int(rect.height/4), Scalar(0,0,maxx), -1);
}
...
bool ArmorDetector::DetectArmor(Mat &img, Rect roi_rect)
{
    ...
    threshold(gray, binary_brightness_img, gray_th_, maxx, THRESH_BINARY);
    threshold(result_img, binary_color_img, color_th_, maxx, THRESH_BINARY);
    imshow("gray",binary_brightness_img);
    ...
    for(size_t i = 0; i < LED_Stick_v.size() ; i++){//去重
...
//#endif
    ...
    if(found_flag){
        ...
        for (int i = 0; i < 4 ; i++){
            line(src, points_roi_tmp[i], points_roi_tmp[(i+1)%4], Scalar(maxx,maxx,0),1);
        }
        ...
        //计算ROI的相关参数
        points_roi_tmp.clear();
        rectangle(img, last_target_,Scalar(maxx,maxx,maxx), 1);
        lost_cnt_ = 0;
    }
    else{
        //计算ROI的相关参数
...
void brightAdjust(Mat src, Mat dst, double dBright){
    int nVal;
...
ArmorDetector pic;
int main(){
    VideoCapture capture("ap.avi");
    while(1){
        capture >> src;
        //bilateralFilter(src,src,10,100,10);
        src_clone = src.clone();
        brightAdjust(src, src_clone, 170);
        Rect roi = pic.GetRoi(src);
        pic.DetectArmor(src_clone,roi);
