...
void brightAdjust(Mat img, Mat dst, double dContrast, double dBright)//调亮度
{
    int nVal;
...
armor::armor(const LED_Stick& L1, const LED_Stick& L2){
    Led_stick[0]= L1;
    Led_stick[1]= L2;
    error_angle = fabs(L1.rect.angle - L2.rect.angle);//角度误差
...
void armor::draw_rect( Mat& img, Point2f roi_offset_point) const//内框
{
...
Rect ArmorDetector::GetRoi(Mat &img)
{
...
bool ArmorDetector::DetectArmor(Mat &img,Rect roi_rect)
{
    ...
    Mat result_img;
    vector<Mat> channels;//分离通道
    split(img,channels);
    Mat channel_img;
    cvtColor(img,gray,COLOR_BGR2GRAY);
    if(color_ == 0)
    {
        subtract(channels[2],channels[1],channel_img);
    }else
    {
        subtract(channels[0],channels[2],channel_img);
    }

    Mat kernel=getStructuringElement(MORPH_RECT,Size(7,7));
    morphologyEx(channel_img,result_img,MORPH_CLOSE,kernel);
    threshold(gray, binary_brightness_img, 60, 255, THRESH_BINARY);
    threshold(result_img, binary_color_img, 15, 255, THRESH_BINARY);
...
//#endif
    ...
    for(size_t i = 0; i < contours_brightness.size(); i++)
    {
        ...
        {
            if(pointPolygonTest(contours_light[ii], contours_brightness[i][0], false) >= 0.0 )//检测点是否在轮廓内
            {
...
int main()
{
    ...
    {cap >>img;
     if(img.empty())
     break;
    brightAdjust(img,img,1,-150);
    a.GetRoi(img);
    a.DetectArmor(img,roi);
