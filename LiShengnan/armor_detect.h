...
public:
    ...
    size_t match_index; // 匹配对应的灯条序号， 用于灯条匹配//定义在cstddef头文件
...
public:
    ...
    void draw_rect(Mat& img, Point2f roi_offset_point) const;    // 画出装甲板
    ...
    bool is_suitable_size(void) const;          // 判断可能的装甲板是否符合尺寸      //void在函数括号内：该函数的参数为空，或者说该函数不带参数
...
public:
    ...
    bool makeRectSafe(cv::Rect & rect, cv::Size size)//cv::Size类中两个成员为width,hight
    {
        if (rect.x < 0)
            rect.x = 0;
