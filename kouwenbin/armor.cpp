...
{
    Led_stick[0]= L1;
    Led_stick[1]= L2;//灯条来源未知
    error_angle = fabs(L1.rect.angle - L2.rect.angle);//取差的绝对值,判断灯条是否平行，查看角度

    rect.width = abs(static_cast<int>(L1.rect.center.x - L2.rect.center.x));//取绝对值，强制类型转换，取俩灯条的中心距离，和宽度
    rect.height = static_cast<int>((L1.rect.size.height + L1.rect.size.height)/2);//取得平均距离
    center.x = static_cast<int>((L1.rect.center.x + L2.rect.center.x)/2);//取得坐标，作用？
...
void armor::draw_rect( Mat& img, Point2f roi_offset_point) const    //画圆和矩形，roi_offset_point的图形，在哪不知道
{
...
int armor::get_average_intensity(const Mat& img) {
    if(rect.width < 1 || rect.height < 1 || rect.x < 1 || rect.y < 1
            || rect.width + rect.x > img.cols || rect.height + rect.y > img.rows)//矫正
        return 255;
    Mat roi = img(Range(rect.y, rect.y + rect.height), Range(rect.x, rect.x + rect.width) );
    //        imshow("roi ", roi);
    average_intensity = static_cast<int>(mean(roi).val[0]);//b图像的均值 ，获得第一个图像的均值。，计算roi的均值
    return average_intensity;
}


void armor::max_match(vector<LED_Stick>& LED,size_t i,size_t j){//匹配左右
    RotatedRect R, L;
    ...
    if(!LED.at(i).matched && !LED.at(j).matched )//led匹配两个都不是，就
    {
    ...
    if(LED.at(i).matched && !LED.at(j).matched)
    {
        ...
        {
            LED.at(LED.at(i).match_index).matched = false;        //第i个东西的容器存放的不知道什么东西，但是把匹配因数放在了i，不知道为什么//匹配个体代表变零
            LED.at(i).match_factor = f;
            LED.at(i).match_index = j;
            LED.at(j).matched = true;
            LED.at(j).match_factor = f;
...
bool armor::is_suitable_size(void) const
{
    ...
    if(Led_stick[0].rect.size.height*0.7f < Led_stick[1].rect.size.height
            && Led_stick[0].rect.size.height*1.3f > Led_stick[1].rect.size.height)//各种条件体积判断计算//
    {
...
Rect ArmorDetector::GetRoi(const Mat &img)
{
    ...
    if(rect_tmp.x == 0 || rect_tmp.y == 0
            ...
            || lost_cnt_ >= 15 || detect_cnt_%100 == 0                        //得到roi，防止越界。
    ...
    else
    {
        ...
        if (lost_cnt_ < 30)
            scale = 3;
        else if(lost_cnt_ <= 60)////设立不同等级干啥？不同等级来扩大roi范围
            scale = 4;
...
Mat bright_adjust(Mat frame)
{
    ...
    for (int i = 0; i < frame.rows; i++)
    {
        ...
        {
            //也可以用for循环
            dstImage.at<Vec3b>(i, j)[0] = saturate_cast<uchar>(frame.at<Vec3b>(i, j)[0] * contrast*0.01 + bright);
            dstImage.at<Vec3b>(i, j)[1] = saturate_cast<uchar>(frame.at<Vec3b>(i, j)[1] * contrast*0.01 + bright);
            dstImage.at<Vec3b>(i, j)[2] = saturate_cast<uchar>(frame.at<Vec3b>(i, j)[2] * contrast*0.025 + bright);
        }
    }
    // frame.convertTo(dstImage, -1, 0.01, 20);
    return dstImage;
}
Mat bright_decrease(Mat image)
{
// 
    float alpha=0.1;
    int beta=20;
...
int main(int argc, char** argv)
{
    ...
    capture.open("/home/kwb/zuoye/amor/111.mp4");
    if(!capture.isOpened())
    {
    ...
    while(1)
    {
        ...
        video =  bright_adjust(video);
        Rect a = arm_det.GetRoi(video);
        arm_det.DetectArmor(video,a);
