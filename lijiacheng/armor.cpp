...
using namespace cv;
int color_;
void ab(Mat src, Mat dst, double dcontrast, double dbright)
{

    int a;
    for(int i = 0;i < src.rows; i++)
    {
        Vec3b* p1 = src.ptr<Vec3b>(i);
        Vec3b* p2 = dst.ptr<Vec3b>(i);
        for(int j =0;j < src.cols; j++)
        {
            for(int n =0;  n < 3; n++)
            {
                a = (int)(dcontrast * p1[j][n] + dbright);
                if(a < 0)
                    a = 0;
                if(a > 255)
                    a = 255;
...
bool ArmorDetector::DetectArmor(Mat &img, Rect roi_rect)
{
    ...
    if(color_ == 0)
    {
        subtract(bgr[2], bgr[1], result_img);//r -b
    }
    else
    {
        subtract(bgr[0], bgr[2], result_img);//b - r
    }
...
//            circle(img, points_2d_.at(i), 3, Scalar(i*50, i*50, 255), -1);
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            line(img,points_roi_tmp[i],points_roi_tmp[(i+1)%4],Scalar(233,233,233),2);
        }
...
{   Mat img;
    ArmorDetector a;
    cin >> color_;
    string path = "/home/aa305/Desktop/picture/armor.mp4";
    VideoCapture cap(0);
