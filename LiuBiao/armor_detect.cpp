...
void armor::max_match(vector<LED_Stick>& LED,size_t i,size_t j){
    ...
    if(!LED.at(i).matched && !LED.at(j).matched ) //容器最初无配对的等条，找到两个就直接配对 （matched = ture）
    {
    ...
    if(LED.at(i).matched && !LED.at(j).matched) //其中一个灯条有配对的灯条，另一个没有
                                                //如果当前两个的灯条拟合出的旋转矩形的误差角度小于之前配对的误差角度
                                                //当前两个灯条配对成功，同时之前有配对的灯条的配对灯条改为未配对状态（matched = false)
    {
...
bool ArmorDetector::DetectArmor(Mat &img, Rect roi_rect)
{
    ...
    vector<LED_Stick> LED_Stick_v;  //用于存储找到的可能的灯条
    ...
    for(size_t i = 0; i < contours_brightness.size(); i++)
    {
        ...
        if (area < 5.0 || 1e5 < area) continue;                 //第一层筛选：面积在一定范围
        for(size_t ii = 0; ii < contours_light.size(); ii++)
        {
            if(pointPolygonTest(contours_light[ii], contours_brightness[i][0], false) >= 0.0 )
            {
                double length = arcLength(contours_brightness[i], true); // 第二层：灯条周长在一定范围
                if (length > 5 && length <4000)
                {          
                    RotatedRect RRect = fitEllipse(contours_brightness[i]); //椭圆拟合效果更好
...
#endif
                    if (fabs(RRect.angle) <= 30)  // 第三层筛选：超过一定角度的灯条不要
                    {
                        LED_Stick r(RRect);
    ...
    for(size_t i = 0; i < LED_Stick_v.size() ; i++)
    {
        for(size_t j = i + 1; j < LED_Stick_v.size() ; j++)
        {
            ...
            if (arm_tmp.error_angle < 8.0f)     //第四层筛选：两等条误差角度小于某一值
            {
                ...
                {
                    if(arm_tmp.get_average_intensity(gray)< 255 )
                    {
    ...
    for(size_t i = 0; i < LED_Stick_v.size() ; i++)
    {
        ...
        {
            LED_Stick_v.at(LED_Stick_v.at(i).match_index).matched = false; //清除之前与之匹配的灯条序号
    ...
    if(found_flag)
    {
        ...
        for(int i=0;i<4;i++)
        {
            points_roi_tmp.push_back(point_2d[i]);
