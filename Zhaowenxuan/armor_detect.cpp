...
armor::armor(const LED_Stick& L1, const LED_Stick& L2){
    Led_stick[0]= L1;
    Led_stick[1]= L2;
    error_angle = fabs(L1.rect.angle - L2.rect.angle);//浮点型绝对值

    rect.width = abs(static_cast<int>(L1.rect.center.x - L2.rect.center.x));//整型绝对值
...
bool ArmorDetector::DetectArmor(Mat &img, Rect roi_rect)
{
    ...
    rectangle(img, roi_rect,Scalar(255, 0, 255),1);//img
#endif
...
//            circle(img, points_2d_.at(i), 3, Scalar(i*50, i*50, 255), -1);
        ...
        rectangle(img, last_target_,Scalar(255,255,255), 1);//img
        lost_cnt_ = 0;
    }else {
        //计算ROI的相关参数
        lost_cnt_ ++;
